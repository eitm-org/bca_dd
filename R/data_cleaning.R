...
df <- merge(df0, ctg_df[,c("well", "sheet_name", "treatment_duration", "signal")], 
            ...
            suffixes = c("", "_ctg")) %>%
  #group_by plate
  group_by(sheet_name, operator, cell_line, treatment_duration) %>%
  ...
  group_by(plate_id, operator, cell_line) %>%
  mutate(d0_avg_nn_tla = mean(total_live_area_nn[treatment_duration == 0], na.rm = TRUE)) %>%
  #group by plate AND condition
  group_by(plate_id, operator, treatment_duration, condition, drug_concentration, cell_line) %>%
  ...
  mutate(percent_control_ctg = (signal - lctrl_avg_ctg)/(hctrl_avg_ctg - lctrl_avg_ctg)*100,
         ...
         percent_control_nn_tla = (total_live_area_nn - hctrl_avg_nn_tla)/(lctrl_avg_nn_tla - hctrl_avg_nn_tla)*100,
         plate_zprime_nn_tla = 1 - (3*lctrl_sd_nn_tla + 3*hctrl_sd_nn_tla)/abs(lctrl_avg_nn_tla - hctrl_avg_nn_tla)) %>%
  #group by plate and condition
  group_by(sheet_name, condition, drug_concentration, treatment_duration) %>%
...
############################
checker_df <- df %>%
  dplyr::select(well, sheet_name_ctg, treatment_duration, signal_ctg, object_count_number_dead_nn) %>%
  distinct() %>%
  arrange(sheet_name_ctg, well, treatment_duration)
checker_nn_tla_df <- nn_df %>%
  dplyr::select(well, ctg_merge_id, treatment_duration) %>%
  distinct() %>%
  arrange(ctg_merge_id, well, treatment_duration)
checker_zprime <- df %>%
  dplyr::select(sheet_name_ctg, treatment_duration, seeding_date_ctg, seeding_date_nn, plate_zprime_ctg, plate_zprime_nn_tla)
checker_cv <- df %>%
  dplyr::select(sheet_name_ctg, treatment_duration, condition_ctg, percent_cv_ctg, plate_percent_cv_ctg, percent_cv_nn_tla, plate_percent_cv_nn_tla)
checker_gr <- df %>%
  dplyr::select(sheet_name_ctg, treatment_duration, d0_avg_nn_tla, gr_nn_tla)
