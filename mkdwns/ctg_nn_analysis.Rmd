...
```{r include = FALSE}
ctgplot0 <- ggplot(data = df[df$treatment_duration == 5,], aes(x = percent_control_ctg, y = percent_control_nn_tla)) +
  facet_wrap(~nn) +
  geom_point(alpha = .5) +
...
```{r ctg_v_nn_print, fig.height = 4, fig.width = 6, out.width = "40%"}
ctgvnn_plot
knitr::include_graphics("/Users/acoleman/Documents/GitHub/bca_dd/images/tme_nn_results.png")
```
\n

```{r ctgvnn_raw, fig.height = 4, out.width = "40%"}
ctg_nn_raw
```
...
```{r zprime_plot, fig.height = 7, fig.width =15, results = "asis"}
for_z_plot <- df %>%
...
sample_only <- df %>%
  ...
  filter(treatment_duration == max(treatment_duration)) %>%
  filter(nn == "NewNN")
...
```{r drcs_print-plots, fig.height = 6, fig.width = 7, out.width = "40%", results = "asis"}
#print plots_output
...
```{r gr, fig.height = 5, fig.width = 7, out.width = "50%", results = "asis"}
df1 <- df %>%
...
```{r gr2, fig.height = 5, fig.width = 7, out.width = "50%", results = "asis"}
